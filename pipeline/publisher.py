...
# Canadian subdivisions. None of these collide with a US postal
# abbreviation, so one appearing anywhere in the string is clean
# evidence for Canada even when the country itself is only ``CA``.
_CA_PROVINCE_RE = re.compile(
    ...
    r"newfoundland|nova scotia|nunavut|ontario|prince edward island|"
    r"quebec|québec|saskatchewan|yukon)\b",
    re.IGNORECASE,
)


def _resolve_ca(location: str) -> str:
    """``CA`` is both California and Canada.

    The audit found 24 of 80 San Francisco postings filed under country
    Canada, which silently removed them from a US-filtered search. Bare
    ``"<City>, CA"`` is overwhelmingly a US address — every Canadian
    source we carry writes the country out ("Toronto, ON, Canada"),
    which the country-name pass catches long before this one. So the
    tie goes to California unless a province says otherwise.
    """
    return "CA" if _CA_PROVINCE_RE.search(location) else "US"


def _resolve_de(location: str) -> str:
    """``DE`` is both Delaware and Germany, and Germany wins.

    The reverse of :func:`_resolve_ca`: ``"<City>, DE"`` is the alpha-2
    suffix the EU ATSes (SmartRecruiters, Recruitee) emit for German
    postings, and that is the high-volume case. Delaware addresses in
    this bare form are consequently misfiled as German — an accepted
    residual, since a US posting that also names its country or a second
    state is already resolved above.
    """
    return "DE"


# Trailing alpha-2 tokens that are simultaneously an ISO country code
# and a US postal abbreviation, and so cannot be read as a country
# without deciding which meaning dominates in our sources.
_AMBIGUOUS_TRAILING_CODES: dict[str, Callable[[str], str]] = {
    "CA": _resolve_ca,
    "DE": _resolve_de,
}
...
def _country_iso_from_location(loc: object) -> str:
    ...
    A trailing two-letter token that is also a US postal abbreviation is
    resolved by :data:`_AMBIGUOUS_TRAILING_CODES` rather than being taken
    as a country outright — see that table for why ``CA`` and ``DE``
    disagree. Anything with no country token at all still falls through
    to :func:`looks_us`, which reads US states the country-name list
    cannot ("Austin, TX").
    """
    ...
    if m:
        ...
        if code in _AMBIGUOUS_TRAILING_CODES:
            return _AMBIGUOUS_TRAILING_CODES[code](stripped)
        if code in _COUNTRY_CODES:
            return code
    if looks_us(stripped):
        return "US"
    return ""
