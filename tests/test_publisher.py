...
def ats_csv_dir_phase2(tmp_path):
    ...
    ``"Sr. Backend Engineer (m/w/d)"`` — same role, abbreviated on one
    side. ``token_sort_ratio`` scores the pair 92.6, over Phase 2's
    default threshold of 90, while the differing titles keep Pass 2 and
    Pass 4 from pairing them.
    """
    eures_dir = tmp_path / "eures"
    eures_dir.mkdir()
    pd.DataFrame([{
...
def test_country_iso_keeps_germany_for_trailing_de():
    ...
    ``"<City>, DE"`` is the alpha-2 suffix the EU ATSes emit, so Germany
    is the default. Delaware is no longer an accepted residual: the
    handful of city names that carry the state are anchored, so only an
    unrecognised city falls through to Germany.
    """
    from pipeline.publisher import _country_iso_from_location as f

    assert f("Berlin, DE") == "DE"
    assert f("Hamburg, DE") == "DE"
    # Delaware's own towns are anchored and resolve to the US.
    assert f("Wilmington, DE") == "US"
    assert f("Dover, DE") == "US"
